...
(function($) {
	$.fn.linkPreview = function(options) {
        ...
        $(this).append('<div id="previewLoading' + selector + '" class="previewLoading"></div> <input type="text" id="text'+selector+'" placeholder="'+opts.placeholder + opts.objectName +'" class="text"/></input> <div id="postPreview'+selector+'" class="postPreview"> <button type="button" id="postPreviewButton'+selector+'" class="btn btn-default btn-success has-spinner postPreviewButton"> <span class="spinner"><i class="icon-spin icon-refresh"></i></span> Comment</button></div>  <div id="preview'+selector+'" class="preview"> <div id="previewImages'+selector+'" class="previewImages"> <div id="previewImage'+selector+'" class="previewImage"><img src="https://www.burnscowboyshop.com/App_Themes/desktop/images/horizontal-loading.gif"/> </div> <input type="hidden" id="photoNumber'+selector+'" class="photoNumber" value="0" /> </div> <div id="previewContent'+selector+'" class="previewContent"> <div id="closePreview'+selector+'" title="Remove" class="closePreview" ></div> <div id="previewTitle'+selector+'" class="previewTitle"></div> <div id="previewUrl'+selector+'" class="previewUrl"></div> <div id="previewDescription'+selector+'" class="previewDescription"></div> <div id="hiddenDescription'+selector+'" class="hiddenDescription"></div> <div id="previewButtons'+selector+'" class="previewButtons" > <div id="previewPreviousImg'+selector+'" class="buttonLeftDeactive" ></div> <div id="previewNextImg'+selector+'" class="buttonRightDeactive" ></div> <div id="photoNumbers'+selector+'" class="photoNumbers" ></div> <div id="chooseThumbnail'+selector+'" class="chooseThumbnail"> Choose a thumbnail </div> </div> </div> </div> </div><div class="previewPostedList" id="previewPostedList'+selector+'"></div> <div id ="commentExtra" > </div>');
		...
		var crawlText = function () {
			...
			if (block === false && urlRegex.test(text)) {
				...
				myThis.find('#previewLoading'+selector).html("<img src='https://www.burnscowboyshop.com/App_Themes/desktop/images/horizontal-loading.gif' />");
				...
				$.post('Facebook-Link-Preview-master/php/textCrawler.php', {
					...
					myThis.find('#previewImage'+selector).html("<a href='" + answer.pageUrl + "' target='_blank'>" + appendImage + "</a><div id='whiteImage' display:none;'>...</div>");
		...
		myThis.find('#postPreview'+selector).click(function() {
			...
			if (((trim(text) !== "") || (trim(text) === "" && trim(hrefUrl) !== "")) && (allowPosting === true && isCrawling === false)) {
				$.get('Facebook-Link-Preview-master/php/highlightUrls.php', {
					...
					if (myThis.find('#noThumb'+selector).attr("checked") === "checked" || images.length === 0) {
						...
						if (video === "yes") {
							...
							pDP = "pDP" + imageId;
							imageId = "img" + imageId;
							image = "<img id='" + imageId + "' src='" + myThis.find('#imagePreview'+ selector + '_' + photoNumber).attr("src") + "' class='imgIframe' ></img>";
							videoPlay = '<span class="videoPostPlay"></span>';
							leftSideContent = image + videoPlay;
						} else {
							image = "<img src='" + myThis.find('#imagePreview'+ selector + '_' + photoNumber).attr("src") + "</img>";
							leftSideContent = '<a href="' + hrefUrl + '" target="_blank">' + image + '</a>';
						}
					}
					content = '<div class="previewPosted">' + '<div class="previewTextPosted">' + urls.urls + '</div>' + videoIframe + '<div class="previewImagesPosted">' + '<div class="previewImagePosted">' + leftSideContent + '</div>' + '</div>' + '<div class="previewContentPosted">' + '<div class="previewTitlePosted" id="' + pTP + '">' + '<a href="' + hrefUrl + '" target="_blank">' + title + '</a></div>' + '<div class="previewUrlPosted">' + fancyUrl + '</div>' + '<div class="previewDescriptionPosted" id="' + pDP + '">' + urls.description + '</div>' + '</div>' + '</div>';
                ...
                {
                	var richComment = '<div class="previewPosted"><div class="previewImagesPosted"><div class="previewImagePosted"><a href="'+itemUrl+'" target="_blank"><img src="'+itemImage+'"></a></div></div><div class="previewContentPosted"><div class="previewTitlePosted" ><a href="'+itemUrl+'" target="_blank"><span id="previewSpanTitle">'+itemTitle+'</span></a></div><div class="previewUrlPosted">'+itemCanonicalUrl+'</div><div class="previewDescriptionPosted"  > <span id="previewSpanDescription">'+itemDescription+'</span></div></div>';
                	if(!itemImage) {
                		richComment = null;
